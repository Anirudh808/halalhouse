...
const statusConfig = {
  new: { label: 'Initiated', color: 'bg-blue-500/10 text-blue-400 border border-blue-500/20', icon: Package },
  preparing: { label: 'In Synthesis', color: 'bg-yellow-500/10 text-yellow-500 border border-yellow-500/20', icon: Clock },
  ready: { label: 'Ready for Pickup', color: 'bg-emerald-500/10 text-heading-gold border border-heading-gold/30', icon: CheckCircle2 },
  completed: { label: 'Fulfilled', color: 'bg-neutral-800 text-neutral-400 border border-neutral-700', icon: CheckCircle2 },
};
    ...
    <div className="min-h-screen bg-background pb-12 transition-colors duration-300">
      ...
      <div className="bg-white/80 dark:bg-black/90 text-foreground dark:text-heading-gold px-6 py-8 sticky top-0 z-30 shadow-xl backdrop-blur-2xl border-b border-border/50 dark:border-neutral-900 transition-all duration-500 rounded-b-[40px]">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tighter uppercase italic">Ledger</h1>
            <p className="text-muted-foreground dark:text-muted-foreground text-[10px] font-black uppercase tracking-[0.2em] opacity-80 transition-colors">Historical Order Stream</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-10 space-y-8">
        {mockOrders.length === 0 ? (
          <div className="text-center py-24 animate-in fade-in zoom-in duration-700">
            ...
            <h2 className="text-3xl font-black text-foreground dark:text-white mb-4 tracking-tighter uppercase italic transition-colors">No records found</h2>
            <p className="text-muted-foreground dark:text-muted-foreground font-bold mb-12 max-w-xs mx-auto italic uppercase text-[10px] tracking-widest opacity-80 leading-relaxed">
              Your future culinary activations will be archived here in the secure ledger.
            </p>
          </div>
        ) : (
          mockOrders.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-card dark:bg-neutral-950 rounded-[40px] shadow-sm border border-border/50 dark:border-neutral-900 overflow-hidden hover:shadow-2xl transition-all duration-500 group active:scale-[0.98]">
                {/* Order Header */}
                <div className="bg-accent/30 dark:bg-neutral-900 px-8 py-6 flex items-center justify-between border-b border-border/30 dark:border-neutral-900 transition-colors">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <p className="font-black text-foreground dark:text-white text-base tracking-tighter uppercase italic leading-none transition-colors mb-1">{order.id}</p>
                  ...
                  <Badge className={`${config.color} border-none font-black text-[9px] py-2 px-4 rounded-xl uppercase tracking-[0.2em] shadow-sm italic transition-all group-hover:scale-105`}>
                ...
                <div className="p-8">
                  <div className="space-y-4 mb-8">
                    {order.items.map((item, idx) => (
                      <div key={idx} className="flex justify-between text-sm transition-all duration-300">
                        <span className="text-foreground/80 dark:text-neutral-300 font-bold flex items-center gap-4 transition-colors">
                          <span className="bg-accent/50 dark:bg-neutral-900 text-foreground dark:text-heading-gold w-8 h-8 flex items-center justify-center rounded-xl font-black text-[11px] border border-border/5 dark:border-neutral-800 italic transition-colors shadow-inner">{item.quantity}</span>
                          <span className="uppercase tracking-tight italic opacity-90">{getProductName(item.productId)}</span>
                        </span>
                  ...
                  <div className="flex items-center justify-between pt-8 border-t border-dashed border-border/30 dark:border-neutral-900 opacity-80">
                    ...
                    <div className="text-right">
                       <p className="text-[9px] text-muted-foreground dark:text-muted-foreground font-black uppercase tracking-[0.3em] mb-1 italic">Settlement Total</p>
                       <p className="font-black text-emerald-600 dark:text-heading-gold text-2xl tracking-tighter italic transition-colors">
                  ...
                  {order.status === 'ready' && (
                    <div className="mt-8 bg-black dark:bg-heading-gold rounded-[30px] p-6 flex items-start gap-5 shadow-2xl animate-in slide-in-from-top-4 duration-700 border-2 border-white/5 dark:border-black/10">
                      ...
                      <div className="text-sm">
                        <p className="font-black text-white dark:text-black uppercase tracking-[0.3em] text-[10px] mb-1 italic">Ready for pickup!</p>
                        <p className="text-emerald-100 dark:text-black/70 font-bold text-xs leading-relaxed italic opacity-90 uppercase tracking-widest">
