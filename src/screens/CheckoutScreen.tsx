    ...
    <div className="min-h-screen bg-background pb-40 transition-colors duration-300">
      ...
      <div className="bg-white/80 dark:bg-black/90 text-foreground dark:text-heading-gold px-6 py-8 sticky top-0 z-40 shadow-xl backdrop-blur-2xl border-b border-border/50 dark:border-neutral-900 transition-all duration-500 rounded-b-[40px]">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tighter uppercase ">Execution</h1>
      ...
      <div className="px-6 py-10">
        <div className="flex items-center gap-4 mb-6 ml-4 opacity-70">
           ...
           <p className="text-[10px] font-black text-muted-foreground dark:text-neutral-500 uppercase tracking-[0.4em]  transition-colors">Manifest Summary</p>
        </div>
        <div className="bg-card dark:bg-neutral-950 rounded-[40px] p-10 shadow-sm border border-border/50 dark:border-neutral-900 space-y-6 transition-colors">
          {restaurantCart.map((item) => (
            <div key={item.product.id} className="flex justify-between items-start animate-in fade-in slide-in-from-right-10 duration-500">
              <div className="flex-1">
                <p className="font-black text-foreground dark:text-white text-base tracking-tighter uppercase  leading-none mb-1 transition-colors">{item.product.name}</p>
              ...
              <p className="font-black text-foreground dark:text-white text-lg tracking-tighter  transition-colors">
          ...
          <div className="pt-8 mt-4 border-t border-dashed border-border/30 dark:border-neutral-800 flex justify-between items-center opacity-80">
             <span className="text-muted-foreground dark:text-muted-foreground text-[10px] font-black uppercase tracking-[0.3em] ">Accumulated Sum</span>
             <span className="font-black text-emerald-600 dark:text-heading-gold text-2xl tracking-tighter  transition-colors">€{total.toFixed(2)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 pb-10">
        <div className="flex items-center gap-4 mb-6 ml-4 opacity-70">
           <div className="h-1 w-10 bg-emerald-600 dark:bg-heading-gold rounded-full"></div>
           <p className="text-[10px] font-black text-muted-foreground dark:text-neutral-500 uppercase tracking-[0.4em]  transition-colors">Pickup Logistics</p>
        </div>
        ...
        <div className="bg-card dark:bg-neutral-950 rounded-[45px] p-10 shadow-sm border border-border/50 dark:border-neutral-900 space-y-10 transition-colors">
          <div className="flex items-start gap-6 group">
            ...
            <div className="flex-1 border-b border-border/20 dark:border-neutral-900 pb-8">
              <p className="font-black text-foreground dark:text-white text-base uppercase tracking-tight mb-3 ">Node Location</p>
              <p className="text-[11px] text-muted-foreground dark:text-muted-foreground font-bold leading-relaxed uppercase tracking-widest opacity-80 decoration-heading-gold ">
          ...
          <div className="flex items-start gap-6 group">
            ...
            <div className="flex-1 border-b border-border/20 dark:border-neutral-900 pb-8">
              <p className="font-black text-foreground dark:text-white text-base uppercase tracking-tight mb-3 ">Synthesis Window</p>
              <p className="text-[11px] text-emerald-600 dark:text-heading-gold font-black tracking-[0.4em] uppercase  transition-colors">ASAP Protocol • 20 Mins</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-6 group">
            ...
            <div className="flex-1">
              <p className="font-black text-foreground dark:text-white text-base uppercase tracking-tight mb-3 ">Settlement Path</p>
              ...
              <div className="flex items-center gap-2 text-emerald-600 dark:text-heading-gold bg-emerald-50 dark:bg-heading-gold/10 w-fit px-4 py-2 rounded-xl border border-emerald-100 dark:border-heading-gold/20 transition-all">
                 ...
                 <span className="text-[9px] font-black uppercase tracking-[0.2em] ">Safeguard Active</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="px-6 pb-12">
        <div className="flex items-center gap-4 mb-6 ml-4 opacity-70">
           <div className="h-1 w-10 bg-emerald-600 dark:bg-heading-gold rounded-full"></div>
           <p className="text-[10px] font-black text-muted-foreground dark:text-neutral-500 uppercase tracking-[0.4em]  transition-colors">Operational Profile</p>
        </div>
        <div className="bg-card dark:bg-neutral-950 rounded-[40px] p-8 shadow-sm border border-border/50 dark:border-neutral-900 flex items-center justify-between transition-colors">
          <div>
            <p className="font-black text-foreground dark:text-white text-xl tracking-tighter uppercase  leading-none mb-1 transition-colors">{currentUser.name}</p>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-black/90 backdrop-blur-3xl border-t border-border/30 dark:border-neutral-900 shadow-[0_-30px_80px_rgba(0,0,0,0.15)] px-10 py-10 z-50 rounded-t-[55px] transition-all duration-700">
        <div className="flex items-center justify-between mb-8">
          <div>
            <span className="block font-black text-foreground dark:text-white text-2xl tracking-tighter uppercase  leading-none mb-1 transition-colors">Final Affirm</span>
            <span className="block text-[8px] text-muted-foreground dark:text-muted-foreground font-black uppercase tracking-[0.4em]  opacity-60">Settlement At Counter</span>
          </div>
          <span className="font-black text-5xl text-emerald-600 dark:text-heading-gold tracking-tighter  drop-shadow-sm transition-colors">€{total.toFixed(2)}</span>
        </div>
        <Button
          onClick={handleConfirmOrder}
          className="w-full h-20 bg-black dark:bg-heading-gold hover:bg-neutral-900 dark:hover:bg-heading-gold/80 text-white dark:text-black text-md font-black rounded-[35px] shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-6 group uppercase tracking-[0.1em] border-2 border-white/5  shadow-black/20"
        >
          Confirm Execution
