...
export function UIViewerLayout() {
  ...
  useEffect(() => {
    const updateScale = () => {
      // 56px top navbar, 32px padding (p-4 top+bottom)
      const availableHeight = window.innerHeight - 56 - 32;
      // 256px sidebar, 32px padding (p-4 left+right)
      const availableWidth = window.innerWidth - 256 - 32;

      const scaleH = availableHeight / 852;
      const scaleW = availableWidth / 393;
        ...
        <div className="flex-1 overflow-y-auto py-3 no-scrollbar">
          <div className="px-3 pb-1">
            <NavLink
              to="/"
              className={({ isActive }) =>
                `flex items-center px-3 py-1.5 text-[14px] rounded-md transition-all ${isActive && location.pathname === "/" ? "bg-indigo-50/50 text-indigo-600 font-medium" : "text-gray-600 hover:bg-gray-50"}`
              }
            >
              home{" "}
              <span className="font-normal ml-1 text-xs text-gray-400">
                (default page)
              </span>
              {location.pathname === "/" && (
      ...
      <div className="flex-1 flex flex-col relative bg-[#f7f7f7]">
        ...
        <div className="h-14 border-b flex items-center justify-between px-6 z-10 w-full shrink-0 bg-white border-gray-200 transition-colors duration-300">
          <div className="flex-1 invisible md:visible">
            <h2 className="text-sm font-semibold text-gray-700 transition-colors duration-300">
              Halal House Showcase
            </h2>
          </div>
          ...
          <div className="flex items-center ml-auto space-x-4">
            <div className="flex items-center space-x-1 border text-[11px] rounded overflow-hidden shadow-sm border-gray-200 text-gray-500 bg-gray-50 transition-colors duration-300">
              <div className="flex items-center px-3 py-1 border-r border-gray-200 transition-colors duration-300">
                <span className="mr-2 font-medium text-gray-400">W</span>{" "}
                <span className="font-semibold text-gray-700">393</span>
              </div>
              <div className="flex items-center px-3 py-1">
                <span className="mr-2 font-medium text-gray-400">H</span>{" "}
                <span className="font-semibold text-gray-700">852</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center overflow-hidden p-4 relative">
          <div
            ...
            style={{
              width: 393,
              height: 852,
              transform: "scale(var(--device-scale, 0.75))",
